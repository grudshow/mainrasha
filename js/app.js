...
// 	document.querySelectorAll(".sort__button").forEach(function (item) {
// 		item.addEventListener("click", function () {
// 			if (window.innerWidth > 767.98 && isMobile.any()) {
// 				if (!item.nextElementSibling.classList.contains("_active")) {
// 					document.querySelector('.sort__list').classList.remove('_spollers');
...
new Swiper('.slider-main__body', {
	...
	navigation: {
		nextEl: '.controls-slider-main__arrow_next',
		prevEl: '.controls-slider-main__arrow_prev',
	},
	breakpoints: {
		767.98: {
			slidesPerView: 2,
...
// 			const matchMedia = window.watchMedia(paramsArray[0]);
...
let _slideToggle = (target, duration = 500) => {
	if (!target.classList.contains('_slide')) {
		// target.classList.add('_slide');
		if (window.getComputedStyle(target).display === 'none') {  //window.getComputedStyle(target).display === 'none'
